...
- `go test ./...` 同时运行纯逻辑单测与 `internal/integration` 集成测试。
- 集成测试（豆种搜索、收藏状态、主页画像、管理员更新/下架）通过 `internal/testsupport` 夹具驱动**完整 Gin 路由**：使用 `t.TempDir()` 下的**文件型 WAL SQLite + 多连接连接池**（非 `:memory:`、非单连接串行化），以真实 HTTP 并发请求压测，并在**连接池层**注入可运行时开关、可恢复的读取故障（`Fault`），结束后关闭服务并可对同一磁盘文件**重启回读**，验证：
  - 名称/描述/风味标签匹配、产地+处理法组合、分页总数与页间不重叠；`%`、`_`、反斜杠与首尾空格按普通文字处理；
  - 收藏态或画像读取失败时，**登录态列表/主页整体失败**（不返回半页、不把收藏误写成未收藏），匿名列表仍可读，故障恢复后回读正确；
  - 重复收藏只产生一条关系，取消后数量/画像回读同步，管理员更新保留收藏、下架事务级联清理。
- 可用 `go test -race -count=N ./internal/integration` 做并发竞争与可重复性检查。
